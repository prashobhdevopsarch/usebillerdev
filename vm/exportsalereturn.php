...
if($fil == 'all')
{
	$bil = $conn->query("SELECT * FROM us_salreturnentry WHERE user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' AND sre_isactive='0' ORDER BY sre_billid DESC");
	...
	$bil = $conn->query("SELECT * FROM us_salreturnentry WHERE DATE(sre_billdate)>='$fromdate' AND DATE(sre_billdate) <= '$todate' AND sre_isactive='0' AND user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' ORDER BY sre_billid DESC");
...
<table id="example" class="display table" style="width: 100%; cellspacing: 0; border-collapse:collapse;" cellpadding="5" border="1">
                                           ...
                                           <tbody>
                                               ...
                                               <tr style="font-size:14px;">
                                                   ...
                                                   <td align="center"><?= $row['sre_billnumber'] ?></td>
                                                   <td align="center">
                                                   <?= date('d-M-Y H:i', strtotime($row['sre_billdate'])) ?>
                                                   </td>
												   <td align="center"><?=$row['sre_rebill']?></td>
