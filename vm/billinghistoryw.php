...
if(isset($_SESSION['admin']))
{
	...
	if(isset($_GET["delete"]))
	{
		...
		if($delete && $slct)
		{
			...
			{
			header('Location:billinghistoryw.php?id=success');
			}
				
			
			else{header('Location:billinghistoryw.php?id=fail');}
		}else{
		  header('Location:billinghistoryw.php?id=fail');
		}
	}
?>
<!DOCTYPE html>
...
<head>  
                        ...
                        <div class="col-lg-12 col-md-12">
							...
							?>
                                <div class="panel-heading">
                                	<a href="exporthistory1w.php?fil=<?= $filt ?>" target="_blank"><button type="button" class="btn btn-primary btn-addon m-b-sm" style="float:right;"><span class="glyphicon glyphicon-print"></span> Print</button></a>
                                    <h4 class="panel-title">Billing History</h4>
                                </div>
                                
                                
                                <div class="panel-body">
                                <form action="billinghistoryw.php" method="post">
								...
								if(isset($_POST['filter']))
											   {
												   $fromdate = $_POST['fromdate'];
												   $todate = $_POST['todate'];
												   $bil = $conn->query("SELECT * FROM us_billentry WHERE DATE(be_billdate)>='$fromdate' AND DATE(be_billdate) <= '$todate' AND be_isactive='0' AND be_mod='2' AND user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' ORDER BY be_billid DESC");
												   echo "<h3>From Date: ".date('d-M-Y', strtotime($fromdate))." &nbsp; &nbsp; To Date: ".date('d-M-Y', strtotime($todate))."</h3>";
											   }
											   else{
											   	$bil = $conn->query("SELECT * FROM us_billentry WHERE be_isactive='0' AND be_mod='2' AND user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' ORDER BY be_billid DESC");
												echo "<h3>ALL BILL DETAILS</h3>";
											   }
								?>
                                    <div class="table-responsive project-stats">  
									<a href="billinghistory.php" ><button type="button" class="btn btn-primary btn-addon m-b-sm" style="float:left;"><span class=""></span>RETAIL</button></a><br><br>
                                    <form class="form-horizontal" method="post" action="exporthistoryw.php?fil=<?= $filt ?>">
	...
	echo $qty = $row2['bi_quantity'];
												   ...
												   <td><?php
													   ...
													   echo $total;
													   ?></td>
                                                   <td><a href="bill_print_A4.php?billid=<?=$row['be_billid']?>&csid=<?=$csid?>&back=<?=$page?>"><span class="glyphicon glyphicon-print"></span> TAX print </a><br>
												   
												  <!-- <a href="bill_print_A5.php?billid=<?=$row['be_billid']?>&csid=<?=$csid?>&back=<?=$page?>"><span class="glyphicon glyphicon-print"></span> TAX print A5 </a><br>-->
                                                  ...
                                                  <a href="editbillW.php?billid=<?=$row['be_billid']?>"><span class="glyphicon glyphicon-edit"></span> edit</a><br>
                                                       <a href="bill_print_A4d.php?billid=<?=$row['be_billid']?>&csid=<?=$csid?>&back=<?=$page?>"><span class="glyphicon glyphicon-print"></span>Dupprint</a><br>
                                                       <a href="bill_print_A4t.php?billid=<?=$row['be_billid']?>&csid=<?=$csid?>&back=<?=$page?>"><span class="glyphicon glyphicon-print"></span>TripPrint</a><br>
                                                 <?php if($row['be_customerid']=='0') {?><a onClick="return confirm('Are you sure you want to delete?')" href="billinghistoryw.php?billid=<?=$row['be_billid']?>&delete"><span class="glyphicon glyphicon-trash"></span> delete</a><?php }?> 
                                                   </td>
                                               </tr>
