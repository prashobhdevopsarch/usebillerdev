...
if($fil == 'all')
{
	$bil = $conn->query("SELECT * FROM us_purreturnentry WHERE user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' AND pre_isactive='0' ORDER BY pre_billid DESC");
	...
	$bil = $conn->query("SELECT * FROM us_purreturnentry WHERE DATE(pre_billdate)>='$fromdate' AND DATE(pre_billdate) <= '$todate' AND pre_isactive='0' AND user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' ORDER BY pre_billid DESC");
...
<table id="example" class="display table" style="width: 100%; cellspacing: 0; border-collapse:collapse;" cellpadding="5" border="1">
                                           ...
                                           <tbody>
                                               ...
                                               <tr style="font-size:14px;">
                                                   ...
                                                   <td align="center"><?= $row['pre_billnumber'] ?></td>
                                                   <td align="center">
                                                   <?= date('d-M-Y H:i', strtotime($row['pre_billdate'])) ?>
                                                   </td>
												  <td align="center"><?= $row['pre_rebill'] ?></td>
