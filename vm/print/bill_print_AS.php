...
<?php if(isset($_GET["back"])){?><a href="<?= $_GET["back"]?>"><button style="float:left;" class="printButtonClass">back</button></a><?php }else{?><a href="servicebill.php"><button style="float:left;" class="printButtonClass">back</button></a><?php }?>
<?php 
session_start();
...
{
	$billid=$_GET["billid"];
	$slct=$conn->query("SELECT * FROM us_deitem WHERE de_billid='$billid'");
	$row=$slct->fetch_assoc();
	
	$cnt=$conn->query("SELECT COUNT(de_billitemid) AS cnt FROM us_deitem WHERE de_billid='$billid'");
...
$bill = $conn->query("SELECT * FROM us_de WHERE de_billid='$billid'");
$row4=$bill->fetch_assoc();
$cust= $conn->query("SELECT * FROM us_customer WHERE cs_customerid='".$row4["de_customerid"]."'");
...
<table  style="width:100%;" align="absmiddle"  border="1">
   ...
   <!-- <th style="width:150px;"></th><th><span>INVOICE</span></th><br>-->
	...
	<b style="float:left">Delivery Address:<?=$row4["de_shipping"]?></b><br>
...
<table border="0" width="100%" class="table">
    ...
    <tr>
    	...
    	<td style="padding:5px; border-left:1px solid #000; border-right:1px solid #000;font-size: 14px;"> <?php if ($row4["de_customerid"] == "0") {
    	?>
        Name & Address : 
         <b><?php
		echo $row4["de_customername"]." &nbsp&nbsp ".$row4["de_address"]; ?></b>
		<br>
		
        
        
        GST Number:  <b><?php echo $row4["de_customer_tin_num"]."&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Tel No. :".$row4["de_customermobile"]; echo "&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Sales person:&nbsp&nbsp"  .$row4["de_saleperson"];
		echo "&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbspDELIVERY DATE : ".$row4["de_deliverydate"]; 
	   } 		
		else {?></b>
       Name & Address : 
        <b><?php
...
$totalbi1=0;
$gk2=0;
$bill2=$conn->query("SELECT * FROM us_deitem a LEFT JOIN us_products b ON b.pr_productid=a.de_productid WHERE a.de_billid='$billid' LIMIT 15");
$tot=0;

while($row6=$bill2->fetch_assoc()){?>
<tr style="text-align:center;">
<?php $disc= $row6['de_disc'];




$am= $disc+$row6['de_taxamount'];?>
   ...
   <!-- <td style="display:none;"><?php echo $row6['pr_productcode'];?></td>-->
    <td align="left" style="width:35%; font-size: 14px;"><span class="s8"><?php  echo $row6["pr_productname"];?></span></td>
	 ...
	 <td><span class="s8"><?php echo $row6['de_quantity'];?></span></td>
...
<tr style="height:30px;">
 ...
 <b>  Recived in good condition: </b><br>
    <br>
...
{
	$r++;	
$slctitm1=$conn->query("SELECT * FROM us_deitem a LEFT JOIN us_products b ON b.pr_productid=a.de_productid WHERE a.de_billid='$billid' LIMIT ".$j.",15");
...
<table  style="width:100%;" align="absmiddle"  border="1">
   ...
   <!-- <th style="width:150px;"></th><th><span>INVOICE</span></th><br>-->
    ...
    <th   align="center" colspan="3" ><!--<span style="font-family:'Times New Roman', Times, serif">--><!--<span class="eras" style="font-family:forte;">--><b><span class="s1"><?=$row3["sp_shopname"]?></span></b><br>
     ...
     Shipping Address : <?=$row4["de_shipping"]?>
...
<table style="text-align: left; border-color:#000;" width="100%" border="1">
   <tr border="1" >
    <td  style="padding:5px;width:50%; border-left:1px solid #000;">
                    ...
                    Invoice Serial Number:  <b><?=$row4["de_billnumber"]?>(<?php $d=date('d-m',strtotime($row4["de_billdate"]));echo $d;?>)</b><br>
                    Invoice Date: <?php $dt=date('d-m-y',strtotime($row4["de_billdate"]));echo $dt?><br>
    ...
    <td  style="padding:5px;width:50%; border-left:1px solid #000">Transportation Mode:<br>
                    Vehicle Number: <?=$row4["de_vehicle_number"]?><br>
                    Date And Time Of Supply:<?php $dt=date('d-m-y',strtotime($row4["de_billdate"]));echo $dt?><br>
                    Place Of Supply: <b><?php if ($row4["de_customerid"] == "0") { 
                         if($row4["de_statecode"]=="AN"){echo "Andaman and Nicobar Islands ";}
                         elseif($row4["de_statecode"]=="AP"){echo "Andhra Pradesh";}
                         elseif($row4["de_statecode"]=="AD"){echo "Andhra Pradesh";}                                                                                         elseif($row4["de_statecode"]=="AR"){echo "Arunachal Pradesh";}
                         elseif($row4["de_statecode"]=="AS"){echo "Assam";}
                         elseif($row4["de_statecode"]=="BH"){echo "Bihar";}
                         elseif($row4["de_statecode"]=="CH"){echo "Chandigarh";}
                         elseif($row4["de_statecode"]=="CT"){echo "Chattisgarh";}
                         elseif($row4["de_statecode"]=="DN"){echo "Dadra and Nagar Haveli";}
                         elseif($row4["de_statecode"]=="DD"){echo "Daman and Diu";}
                         elseif($row4["de_statecode"]=="DL"){echo "Delhi";}
                         elseif($row4["de_statecode"]=="GA"){echo "GoA";}
                         elseif($row4["de_statecode"]=="GJ"){echo "Gujarat";}
                         elseif($row4["de_statecode"]=="HR"){echo "Hariyana";}                           
                         elseif($row4["de_statecode"]=="HP"){echo "Himachal Pradesh";}
                         elseif($row4["de_statecode"]=="JK"){echo "Jammu And Kashmir";}
                         elseif($row4["de_statecode"]=="JH"){echo "Jharkhand";}
                         elseif($row4["de_statecode"]=="KA"){echo "Karnataka";}
                         elseif($row4["de_statecode"]=="KL"){echo "Kerala";}
                         elseif($row4["de_statecode"]=="LD"){echo "LakshadWeep Island";}
                         elseif($row4["de_statecode"]=="MP"){echo "Madhya Pradhesh";}
                         elseif($row4["de_statecode"]=="MH"){echo "Maharastra";}
                         elseif($row4["de_statecode"]=="MN"){echo "Manipur";}
                         elseif($row4["de_statecode"]=="ME"){echo "Meghalaya";}
                         elseif($row4["de_statecode"]=="MI"){echo "Mizoram";}                           
                         elseif($row4["de_statecode"]=="NL"){echo "Nagaland";}
                         elseif($row4["de_statecode"]=="OR"){echo "Odisha";}
                         elseif($row4["de_statecode"]=="PY"){echo "Pondicherry";}
                         elseif($row4["de_statecode"]=="PB"){echo "Punjab";}
                         elseif($row4["de_statecode"]=="RJ"){echo "Rajasthan";}                           
                         elseif($row4["de_statecode"]=="SK"){echo "Sikkim";}
                         elseif($row4["de_statecode"]=="TN"){echo "Tamil Nadu";}
                         elseif($row4["de_statecode"]=="TS"){echo "Telangana";}
                         elseif($row4["de_statecode"]=="TR"){echo "Tripura";}
                         elseif($row4["de_statecode"]=="UP"){echo "Uttar Pradesh";}
                         elseif($row4["de_statecode"]=="UI"){echo "Uttarakhand";}
                         elseif($row4["de_statecode"]=="WB"){echo "West Bengal";}
                         else {echo "Kerala";}}
                    else{
...
<table border="1" width="100%" class="table">
    ...
    <tr>
        ...
        <td style="padding:5px; border-left:1px solid #000;"> <?php if ($row4["de_customerid"] == "0") {
        ?>
        Name:
        <b><?php
        echo $row4["de_customername"];?></b>
        <br>Ph:
        <b><?php
        echo $row4["de_customermobile"]; 
        ?></b>
        <br>State: <b><?php if($row4["de_statecode"]=="AN"){echo "Andaman and Nicobar Islands ";}
                         elseif($row4["de_statecode"]=="AP"){echo "Andhra Pradesh";}
                         elseif($row4["de_statecode"]=="AD"){echo "Andhra Pradesh";}                                                                                         elseif($row4["de_statecode"]=="AR"){echo "Arunachal Pradesh";}
                         elseif($row4["de_statecode"]=="AS"){echo "Assam";}
                         elseif($row4["de_statecode"]=="BH"){echo "Bihar";}
        ...
        <br>State Code:<b><?php if($row4["de_statecode"]==''){echo"32";$stcode_c='KL';}else{$stcode_c=$row4["de_statecode"];switch($row4["de_statecode"])
        {
        ...
        GST Number:  <b><?php echo $row4["de_customer_tin_num"];
        } 
        else {?></b>
        Name:
        <b><?php
        ...
        <br>State Code:<b><?php if($row4["de_statecode"]==''){$stcode_c='KL';echo"32";}else{$stcode_c=$row4["de_statecode"];switch($row4["de_statecode"])
        {
            case 'AN':echo "35";break;
            case 'AP':echo "28";break;
            case 'AD':echo "37";break;
        ...
        else {$cust= $conn->query("SELECT * FROM us_customer WHERE cs_customerid='".$row4["de_customerid"]."'");
        ...
        ?></b>
         ...
         <br>
        State Code: <b><?php //echo $row5["cs_statecode"];
        $row4["de_statecode"]=$row5["cs_statecode"];
        switch($row5["cs_statecode"])
        {
            case 'AN':echo "35";break;
            case 'AP':echo "28";break;
...
$gk2=0;
$tot=0;
$bill2=$conn->query("SELECT * FROM us_deitem a LEFT JOIN us_products b ON b.pr_productid=a.de_productid WHERE a.de_billid='$billid' LIMIT ".$j.",20");
...
<td><span class="s8"><?php echo $row6['pr_unit'];?></span></td>
      ...
      <td><span class="s8"><?php if($row6['de_quantity']!=0) { echo $gk2=number_format(($row6['de_taxamount']+$row6['de_disc'])/$row6['de_quantity'],2); } else { echo 0;}?></span></td>
    <td><span class="s8"><?php echo $gk2*$row6['de_quantity']; ?></span></td>
    <td><span class="s8"><?php echo round($disc,2);?></span></td>
    <td><span class="s8"><?php echo $row6['de_taxamount'];?></span></td>
   
    <?php if($stcode_c==$stcode_s)
    {?>
    <td class="s8"><?php echo $row6['de_cgst'];?></td>
    <td class="s8"><?php $tcgst=$tcgst+round($row6['de_cgst_amt']);echo ($row6['de_cgst_amt']);?></td>
    <td class="s8"><?php echo $row6['de_sgst'];?></td>
    <td class="s8"><?php $tsgst=$tsgst+round($row6['de_sgst_amt']);echo ($row6['de_sgst_amt']);?></td>
    <?php }else{?>
    <td class="s8"><?php echo $row6['de_igst'];?></td>
    <td class="s8"><?php $tigst=$tigst+round($row6['de_igst_amt']);echo ($row6['de_igst_amt']);?></td>
    <?php }?>
<td><span class="s8"><?php echo $row6['de_total'];?></span></td>
   
    
</tr>
<?php $qt=$qt+$row6['de_quantity'];
$amt=$amt+$row6['de_taxamount'];
$dis=$dis+$disc;
$tax=$tax+round($row6['de_taxamount']);
$totalbi2=$totalbi2+$row6['de_total'];?>
<?php $k++; }
while($k<=$j+9)
{?>
...
<?php $slctsgst=$conn->query("select sum(de_cgst_amt) as cgs,sum(de_sgst_amt) as sgs,sum(de_igst_amt) as igs  from us_deitem where de_billid='$billid'"); $rowi=$slctsgst->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="2">
<span class="s8"><?php $slctsgst5t=$conn->query("select sum(de_cgst_amt) as 5tax from us_deitem where de_billid='$billid'"); $rowitm5t=$slctsgst5t->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="2">
<span class="s8"><?php $slctsgst6t=$conn->query("select sum(de_sgst_amt) as 6tax from us_deitem where de_billid='$billid'"); $rowitm6t=$slctsgst6t->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="2">
<span class="s8"><?php $slctsgst7t=$conn->query("select sum(de_igst_amt) as 7tax from us_deitem where de_billid='$billid'"); $rowitm7t=$slctsgst7t->fetch_assoc();
...
<tr>
<td style="border-bottom:1px solid #000;" colspan="2">Other Charges</td><td style="border-bottom:1px solid #000;" colspan="2"><?=$row4["de_coolie"]?></td>
</tr>

<tr>
<td style="border-bottom:1px solid #000;" colspan="2"><b>Invoice Total(Rs) </b></td><td style="border-bottom:1px solid #000;" colspan="2"><b><?= round($totalbi2+$row4["de_coolie"]);?></b></td>
</tr>
<?php
    $billtot = $conn->query("SELECT sum(de_total) as grandtotal FROM us_deitem WHERE de_billid='$billid'");
