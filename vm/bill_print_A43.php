...
<?php if(isset($_GET["back"])){?><a href="<?= $_GET["back"]?>"><button style="float:left;" class="printButtonClass">back</button></a><?php }else{?><a href="performoinvoice.php"><button style="float:left;" class="printButtonClass">back</button></a><?php }?>
<?php 
session_start();
...
{
	$billid=$_GET["billid"];
	$slct=$conn->query("SELECT * FROM us_performo WHERE pe_billid='$billid'");
	$row=$slct->fetch_assoc();
	
	$cnt=$conn->query("SELECT COUNT(pe_billitemid) AS cnt FROM us_perfomoitem WHERE pe_billid='$billid'");
...
$bill = $conn->query("SELECT * FROM us_performo WHERE pe_billid='$billid'");
$row4=$bill->fetch_assoc();
$cust= $conn->query("SELECT * FROM us_customer WHERE cs_customerid='".$row4["pe_customerid"]."'");
...
<table  style="width:100%;" align="absmiddle"  border="1">
   ...
   <!-- <th style="width:150px;"></th><th><span>INVOICE</span></th><br>-->
	...
	<b style="float:left">GSTIN:<?=$row3["sp_tin"]?></b><br>
	<b><span class="s1"><?=$row3["sp_shopname"]?></span></b><br>
 			<?=$row3["sp_shopaddress"]?><br>           
            <?=$row3["sp_email"]?><br>
            <?php $stcode_s=$row3["sp_stcode"];?><br>
			<h4><u>PerformoInvoice</u></h4>
  </th>
  </tr>
...
<table style="text-align: left; border-color:#000;" width="100%" border="1">
   <tr border="1" >
    <td  style="border-left:1px solid #000;">    				
                    <b>Invoice Serial Number:  <?=$row4["pe_billnumber"]?>(<?php $d=date('d-m',strtotime($row4["pe_billdate"]));echo $d;?>)</b>
                  </td><td>  <b>State: <?php 
    ...
    <td>State Code:<b><?php if($row4["pe_statecode"]==''){echo"32";$stcode_c='KL';}else{$stcode_c=$row4["pe_statecode"];switch($row4["pe_statecode"])
		{
	...
	<td>  <b>Invoice Date: <?php $dt=date('d-m-y',strtotime($row4["pe_billdate"]));echo $dt?></b>
                  </td>
  </tr>
</table>
<table border="0" width="100%" class="table">
    ...
    <tr>
    	...
    	<td style="padding:5px; border-left:1px solid #000; border-right:1px solid #000;font-size: 14px;"> <?php if ($row4["pe_customerid"] == "0") {
    	?>
        Name & Address : 
         <b><?php
		echo $row4["pe_customername"]." &nbsp&nbsp ".$row4["pe_address"]; ?></b>
		<br>
		
        
        
        GST Number:  <b><?php echo $row4["pe_customer_tin_num"]."&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Tel No. :".$row4["pe_customermobile"]; echo "&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Sales person:&nbsp&nbsp"  .$row4["pe_saleperson"];
...
<th rowspan="2" bgcolor="#999999" align="left">Name of Product</th>
<th rowspan="2" style="width:75px;" bgcolor="#999999">HSN Code</th>
...
$totalbi1=0;
$gk2=0;
$bill2=$conn->query("SELECT * FROM us_perfomoitem a LEFT JOIN us_products b ON b.pr_productid=a.pe_productid WHERE a.pe_billid='$billid' LIMIT 15");
$tot=0;

while($row6=$bill2->fetch_assoc()){?>
<tr style="text-align:center;">
<?php $disc= $row6['pe_disc'];




$am= $disc+$row6['pe_taxamount'];?>
   ...
   <!-- <td style="display:none;"><?php echo $row6['pr_productcode'];?></td>-->
    ...
    <td><span class="s8"><?php echo $row6['pr_hsn'];?></span></td>
	 ...
	 <td><span class="s8"><?php echo $row6['pe_quantity'];?></span></td>
       <!--<td><span class="s8"><?php if($row6['pe_quantity']!=0) { echo $gk2=number_format(($row6['pe_taxamount']+$row6['pe_disc'])/$row6['pe_quantity'],2); } else { echo 0;}?></span></td>-->
    <td><span class="s8"><?php echo $row6['pe_price'] ?> </span></td>
    <td><span class="s8"><?php echo $row6['pe_price']*$row6['pe_quantity']; ?></span></td>
    <td><span class="s8"><?php echo $row6['pe_disc']; ?></span></td>
    <td><span class="s8"><?php echo $row6['pe_price']*$row6['pe_quantity']+$row6['pe_disc'];?></span></td> 
    <!--<?php $amt1= $row6['pe_price']*$row6['pe_quantity']+$row6['pe_disc'];?>-->
</tr>
<?php $qt=$qt+$row6['pe_quantity'];
$amt=$amt+$row6['pe_taxamount'];
$dis=$dis+$disc;
$tax=$tax+round($row6['pe_taxamount']);
$totalbi1=$totalbi1+$row6['pe_total'];
    // $amt3=$amt3+$amt1;                              
    ?>
<?php $k++; }
while($k<=15)
{?>
    <tr><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;<td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td></tr>
<?php $k++;}?>
...
<td align="center"><?php echo $amt;?></td> 
    <td align="center"><?php echo $tax + $totalbi1;?></td> 
</tr>
</table>
...
<?php $slctsgst=$conn->query("select sum(pe_cgst_amt) as cgs,sum(pe_sgst_amt) as sgs,sum(pe_igst_amt) as igs  from us_perfomoitem where pe_billid='$billid'"); $rowi=$slctsgst->fetch_assoc();
$totax=$rowi["cgs"]+$rowi["sgs"]+$rowi["igs"];?>

<span class="s8">Total Invoice Amount in words : <b> <?php $amd=convertToIndianCurrency(round($totalbi1));echo $amd;?> </b> Only</span>
</td>
...
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8"><?php echo $totalbi1;?></span>
</td>
</tr>
<tr>
<td>
Transport Vehicle No. : <?=$row4["pe_vehicle_number"]?><br>
    <br>
    <b>Shipping Address : <?=$row4["pe_shipping"]?></b><br>
    <b>100% Advance : <?=$row4["pe_hunad"]?></b><br>
...
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8"><?php $slctsgst5t=$conn->query("select sum(pe_cgst_amt) as 5tax from us_perfomoitem where pe_billid='$billid'"); $rowitm5t=$slctsgst5t->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8"><?php $slctsgst6t=$conn->query("select sum(pe_sgst_amt) as 6tax from us_perfomoitem where pe_billid='$billid'"); $rowitm6t=$slctsgst6t->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8"><?php $slcess=$conn->query("select sum(pe_cess) as b6cess from us_perfomoitem where pe_billid='$billid'"); $rowitmcess=$slcess->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8">
<b><?= round($totalbi1+$row4["pe_coolie"]+$cessd+$sgst6t+$sgst6t);?></b>
</span>
</td>
</tr>
...
{
	$r++;	
$slctitm1=$conn->query("SELECT * FROM us_perfomoitem a LEFT JOIN us_products b ON b.pr_productid=a.pe_productid WHERE a.pe_billid='$billid' LIMIT ".$j.",15");
...
<table style="text-align: left; border-color:#000;" width="100%" border="1">
   <tr border="1" >
    <td  style="padding:5px;width:50%; border-left:1px solid #000;">
                    ...
                    Invoice Serial Number:  <b><?=$row4["pe_billnumber"]?>(<?php $d=date('d-m',strtotime($row4["pe_billdate"]));echo $d;?>)</b><br>
                    Invoice Date: <?php $dt=date('d-m-y',strtotime($row4["pe_billdate"]));echo $dt?><br>
    ...
    <td  style="padding:5px;width:50%; border-left:1px solid #000">Transportation Mode:<br>
                    Vehicle Number: <?=$row4["pe_vehicle_number"]?><br>
                    Date And Time Of Supply:<?php $dt=date('d-m-y',strtotime($row4["pe_billdate"]));echo $dt?><br>
                    Place Of Supply: <b><?php if ($row4["pe_customerid"] == "0") { 
                         if($row4["pe_statecode"]=="AN"){echo "Andaman and Nicobar Islands ";}
                         elseif($row4["pe_statecode"]=="AP"){echo "Andhra Pradesh";}
                         elseif($row4["pe_statecode"]=="AD"){echo "Andhra Pradesh";}                                                                                         elseif($row4["pe_statecode"]=="AR"){echo "Arunachal Pradesh";}
                         elseif($row4["pe_statecode"]=="AS"){echo "Assam";}
                         elseif($row4["pe_statecode"]=="BH"){echo "Bihar";}
                         elseif($row4["pe_statecode"]=="CH"){echo "Chandigarh";}
                         elseif($row4["pe_statecode"]=="CT"){echo "Chattisgarh";}
                         elseif($row4["pe_statecode"]=="DN"){echo "Dadra and Nagar Haveli";}
                         elseif($row4["pe_statecode"]=="DD"){echo "Daman and Diu";}
                         elseif($row4["pe_statecode"]=="DL"){echo "Delhi";}
                         elseif($row4["pe_statecode"]=="GA"){echo "GoA";}
                         elseif($row4["pe_statecode"]=="GJ"){echo "Gujarat";}
                         elseif($row4["pe_statecode"]=="HR"){echo "Hariyana";}                           
                         elseif($row4["pe_statecode"]=="HP"){echo "Himachal Pradesh";}
                         elseif($row4["pe_statecode"]=="JK"){echo "Jammu And Kashmir";}
                         elseif($row4["pe_statecode"]=="JH"){echo "Jharkhand";}
                         elseif($row4["pe_statecode"]=="KA"){echo "Karnataka";}
                         elseif($row4["pe_statecode"]=="KL"){echo "Kerala";}
                         elseif($row4["pe_statecode"]=="LD"){echo "LakshadWeep Island";}
                         elseif($row4["pe_statecode"]=="MP"){echo "Madhya Pradhesh";}
                         elseif($row4["pe_statecode"]=="MH"){echo "Maharastra";}
                         elseif($row4["pe_statecode"]=="MN"){echo "Manipur";}
                         elseif($row4["pe_statecode"]=="ME"){echo "Meghalaya";}
                         elseif($row4["pe_statecode"]=="MI"){echo "Mizoram";}                           
                         elseif($row4["pe_statecode"]=="NL"){echo "Nagaland";}
                         elseif($row4["pe_statecode"]=="OR"){echo "Odisha";}
                         elseif($row4["pe_statecode"]=="PY"){echo "Pondicherry";}
                         elseif($row4["pe_statecode"]=="PB"){echo "Punjab";}
                         elseif($row4["pe_statecode"]=="RJ"){echo "Rajasthan";}                           
                         elseif($row4["pe_statecode"]=="SK"){echo "Sikkim";}
                         elseif($row4["pe_statecode"]=="TN"){echo "Tamil Nadu";}
                         elseif($row4["pe_statecode"]=="TS"){echo "Telangana";}
                         elseif($row4["pe_statecode"]=="TR"){echo "Tripura";}
                         elseif($row4["pe_statecode"]=="UP"){echo "Uttar Pradesh";}
                         elseif($row4["pe_statecode"]=="UI"){echo "Uttarakhand";}
                         elseif($row4["pe_statecode"]=="WB"){echo "West Bengal";}
                         else {echo "Kerala";}}
                    else{
...
<table border="1" width="100%" class="table">
    ...
    <tr>
        ...
        <td style="padding:5px; border-left:1px solid #000;"> <?php if ($row4["pe_customerid"] == "0") {
        ?>
        Name:
        <b><?php
        echo $row4["pe_customername"];?></b>
        <br>Ph:
        <b><?php
        echo $row4["pe_customermobile"]; 
        ?></b>
        <br>State: <b><?php if($row4["pe_statecode"]=="AN"){echo "Andaman and Nicobar Islands ";}
                         elseif($row4["pe_statecode"]=="AP"){echo "Andhra Pradesh";}
                         elseif($row4["pe_statecode"]=="AD"){echo "Andhra Pradesh";}                                                                                         elseif($row4["pe_statecode"]=="AR"){echo "Arunachal Pradesh";}
                         elseif($row4["pe_statecode"]=="AS"){echo "Assam";}
                         elseif($row4["pe_statecode"]=="BH"){echo "Bihar";}
        ...
        <br>State Code:<b><?php if($row4["pe_statecode"]==''){echo"32";$stcode_c='KL';}else{$stcode_c=$row4["pe_statecode"];switch($row4["pe_statecode"])
        {
            case 'AN':echo "35";break;
            case 'AP':echo "28";break;
            case 'AD':echo "37";break;
        ...
        GST Number:  <b><?php echo $row4["pe_customer_tin_num"];
        } 
        else {?></b>
        Name:
        <b><?php
        ...
        <br>State Code:<b><?php if($row4["pe_statecode"]==''){$stcode_c='KL';echo"32";}else{$stcode_c=$row4["pe_statecode"];switch($row4["pe_statecode"])
        {
            case 'AN':echo "35";break;
            case 'AP':echo "28";break;
            case 'AD':echo "37";break;
        ...
        else {$cust= $conn->query("SELECT * FROM us_customer WHERE cs_customerid='".$row4["pe_customerid"]."'");
        ...
        ?></b>
         ...
         <br>
        State Code: <b><?php //echo $row5["cs_statecode"];
        $row4["pe_statecode"]=$row5["cs_statecode"];
        switch($row5["cs_statecode"])
        {
            case 'AN':echo "35";break;
            case 'AP':echo "28";break;
...
$gk2=0;
$tot=0;
$bill2=$conn->query("SELECT * FROM us_perfomoitem a LEFT JOIN us_products b ON b.pr_productid=a.pe_productid WHERE a.pe_billid='$billid' LIMIT ".$j.",20");
...
<td><span class="s8"><?php echo $row6['pr_unit'];?></span></td>
      ...
      <td><span class="s8"><?php if($row6['pe_quantity']!=0) { echo $gk2=number_format(($row6['pe_taxamount']+$row6['pe_disc'])/$row6['pe_quantity'],2); } else { echo 0;}?></span></td>
    <td><span class="s8"><?php echo $gk2*$row6['pe_quantity']; ?></span></td>
    <td><span class="s8"><?php echo round($disc,2);?></span></td>
    <td><span class="s8"><?php echo $row6['pe_taxamount'];?></span></td>
   
    <?php if($stcode_c==$stcode_s)
    {?>
    <td class="s8"><?php echo $row6['pe_cgst'];?></td>
    <td class="s8"><?php $tcgst=$tcgst+round($row6['pe_cgst_amt']);echo ($row6['pe_cgst_amt']);?></td>
    <td class="s8"><?php echo $row6['pe_sgst'];?></td>
    <td class="s8"><?php $tsgst=$tsgst+round($row6['pe_sgst_amt']);echo ($row6['pe_sgst_amt']);?></td>
    <?php }else{?>
    <td class="s8"><?php echo $row6['pe_igst'];?></td>
    <td class="s8"><?php $tigst=$tigst+round($row6['pe_igst_amt']);echo ($row6['pe_igst_amt']);?></td>
    <?php }?>
<td><span class="s8"><?php echo $row6['pe_total'];?></span></td>
...
$dis=$dis+$disc;
$tax=$tax+round($row6['pe_taxamount']);
$totalbi2=$totalbi2+$row6['pe_total'];?>
<?php $k++; }
while($k<=$j+9)
{?>
...
<span class="s8">Rupees: <?php $amd=convert_number_to_words(round($totalbi2));echo $amd;?> Only</span>
</td>
...
<td style="border-bottom:1px solid #000;" colspan="2">
<span class="s8"><?php $slctsgst7t=$conn->query("select sum(pe_igst_amt) as 7tax from us_perfomoitem where pe_billid='$billid'"); $rowitm7t=$slctsgst7t->fetch_assoc();
...
<tr>
<td style="border-bottom:1px solid #000;" colspan="2">Other Charges</td><td style="border-bottom:1px solid #000;" colspan="2"><?=$row4["pe_coolie"]?></td>
</tr>

<tr>
<td style="border-bottom:1px solid #000;" colspan="2"><b>Invoice Total(Rs) </b></td><td style="border-bottom:1px solid #000;" colspan="2"><b><?= round($totalbi2+$row4["pe_coolie"]);?></b></td>
</tr>
<?php
    $billtot = $conn->query("SELECT sum(pe_total) as grandtotal FROM us_perfomoitem WHERE pe_billid='$billid'");
