								...
								if(isset($_POST['filter']))
											   {
											   	...
											   	$bil = $conn->query("SELECT * FROM us_billentry WHERE be_isactive='0' AND be_mod='1' AND user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' ORDER BY be_billid DESC");
                                               ...
                                               <td> <input type="checkbox" name="bill[]" id="bill<?=$k?>" value="<?= $row['be_billid'] ?>"></td>
												...
												<!--  -->
												   ...
												   if($row["be_customerid"]!=0){
													    $slctcust=$conn->query("SELECT * FROM us_customer WHERE cs_customerid='".$row['be_customerid']."'");
											$rowcus=$slctcust->fetch_assoc();
												   $ele=$conn->query("select * from us_elec where el_id='".$rowcus["cs_elecid"]."' ");
												   $elec=$ele->fetch_assoc();
												   echo $elec["el_name"];
												   }
												   else{
													echo  $row['be_electrician'];   
												   }
												   ?>
                                                   
                                                   </td>
												 
                                                   <td><?=$row["be_com_amnt"]?></td>
                                                   <td>
                                                   	<?php
                                                 ...
                                                 <?php if($row['be_customerid']=='0') {?><a onClick="return confirm('Are you sure you want to delete?')" href="billinghistory.php?billid=<?=$row['be_billid']?>&delete"><span class="glyphicon glyphicon-trash"></span> delete</a><?php }?> 
                                                   </td>
                                               </tr>
