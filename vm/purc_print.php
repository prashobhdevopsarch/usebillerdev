...
{
	$billid=$_GET["billid"];
	$slct=$conn->query("SELECT * FROM us_purentry WHERE pe_billid='$billid'");
...
<?php $slctsup=$conn->query("SELECT * FROM us_supplier WHERE rs_supplierid='".$row["pe_supplierid"]."'");
...
$slctitm=$conn->query("SELECT * FROM us_puritems a LEFT JOIN us_products b ON b.pr_productid=a.pi_productid WHERE a.pi_billid='$billid'");
...
<tr>
 ...
 <td colspan="6" align="left"> Purchase Note: <?=$row["pe_note"]?> </td>
 <td colspan="11" align="center">Grand Total(in words) : <?php $amd=convert_number_to_words($row["pe_gtotal"]);echo $amd;?></td>
