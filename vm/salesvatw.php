        ...
        <main class="page-content content-wrap">
            ...
            <div class="page-inner">
                <div class="page-title">
                    <h3><strong>Wholsale TAX</strong></h3>
                    <div class="page-breadcrumb">
                        <ol class="breadcrumb">
                            <li>Billing</li>
							<li>Wholsale Tax Report</li>
                            <li class="active">Wholsale TAX</li>
                        </ol>
                    </div>
                </div>
                <?php
                        ...
                        <div class="col-lg-12 col-md-12">
							...
							?>
                                <div class="panel-heading">
                                	<a href="exportDBW.php?fil=<?= $filt ?>" target="_blank"><button type="button" class="btn btn-primary btn-addon m-b-sm" style="float:right;margin-left: 5px;"><span class="glyphicon glyphicon-print"></span> Export(TAX%)</button></a>&nbsp;&nbsp;<a href="exportwtax.php?fil=<?= $filt ?>" target="_blank"><button type="button" class="btn btn-primary btn-addon m-b-sm" style="float:right;"><span class="glyphicon glyphicon-print"></span> Export(Item)</button></a>
                                   
                                    
                                    <h4 class="panel-title">Wholesale TAX Report</h4>
                                </div>
								...
								if(isset($_POST['filter']))
											   {
												   $fromdate = $_POST['fromdate'];
												   $todate = $_POST['todate'];
												   $bil = $conn->query("SELECT * FROM us_billentry WHERE DATE(be_billdate)>='$fromdate' AND DATE(be_billdate) <= '$todate' AND be_isactive='0' AND be_mod='2' AND user_id='".$_SESSION["admin"]."' ORDER BY be_billid DESC");
												   echo "<h3>From Date: ".date('d-M-Y', strtotime($fromdate))." &nbsp; &nbsp; To Date: ".date('d-M-Y', strtotime($todate))."</h3>";
											   }
											   else{
											   	$bil = $conn->query("SELECT * FROM us_billentry WHERE be_isactive='0' AND be_mod='2' AND user_id='".$_SESSION["admin"]."' ORDER BY be_billid DESC");
