...
{
	$billid=$_GET["billid"];
	$slct=$conn->query("SELECT * FROM us_purreturnentry WHERE pre_billid='$billid'");
...
<table style="text-align:center;" width="100%" border="0">
 ...
 <!-- <tr>
  ...
  <tr><th>Purchase Return(Debit Note)</th></tr>
  <tr><td>Return Bill No :<?=$row["pre_rebill"]?></td></tr>
</table>


<table style="text-align:left;float:left;" width="40%" border="0">
<?php $slctsup=$conn->query("SELECT * FROM us_supplier WHERE rs_supplierid='".$row["pre_customerid"]."'");
...
$slctitm=$conn->query("SELECT * FROM us_purreturnitem a LEFT JOIN us_products b ON b.pr_productid=a.pri_productid WHERE a.pri_billid='$billid'");
