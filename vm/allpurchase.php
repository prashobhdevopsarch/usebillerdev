...
if(isset($_SESSION['admin']))
{
	...
	if(isset($_GET["delete"]))
	{
		...
		if($delete && $slct)
		{
			...
			{
			header('Location:allpurchase.php?id=success');
			}
		}else{
		  header('Location:allpurchase.php?id=fail');
		}
	}
?>
<!DOCTYPE html>
        ...
        <main class="page-content content-wrap">
            ...
            <div class="page-inner">
                <div class="page-title">
                    <h3><strong>TOTAL PURCHASE REPORT</strong></h3>
                    <div class="page-breadcrumb">
                        <ol class="breadcrumb">
                            <li>Billing</li>
                            <li class="active">TOTAL PURCHASE REPORT</li>
                        </ol>
                    </div>
                </div>
                <?php
                ...
                <div id="main-wrapper">
							...
							?>
                                <div class="panel-heading">
                                 ...
                                 <a href="reports.php" target="_blank"><button type="button" class="btn btn-primary btn-addon m-b-sm" style="float:right;"><span class="fa fa-backward"></span> Back</button></a>
                                    ...
                                    <h4 class="panel-title">TOTAL PURCHASE REPORT</h4>
                                </div>
                                
                                
                                <div class="panel-body">
                                <form action="allpurchase.php" method="post">
                  ...
                  <?php } ?>
								...
								if(isset($_POST['filter']))
											   {
												   $years = $_POST['fyear'];
												   $bil = $conn->query("SELECT count(`pe_billnumber`) as cnt,SUM(`pe_gtotal`) as sm,EXTRACT(YEAR FROM pe_billdate) as yr,EXTRACT(MONTH FROM pe_billdate) as mnth FROM us_purentry WHERE pe_isactive='0'  and finyear='$years' Group BY EXTRACT(MONTH FROM pe_billdate),EXTRACT(YEAR FROM pe_billdate) ORDER BY pe_billdate DESC");


                            $bil1 = $conn->query("SELECT count(`pe_billnumber`) as cnt,SUM(`pe_gtotal`) as sm FROM us_purentry WHERE pe_isactive='0'  and finyear='$years' and pe_balance>0  ORDER BY pe_billdate DESC");
                             $bil2 = $conn->query("SELECT count(`pe_billnumber`) as cnt,SUM(`pe_gtotal`) as sm FROM us_purentry WHERE pe_isactive='0'  and finyear='$years' and pe_balance=0  ORDER BY pe_billdate DESC");


												     //echo "<h3>Year: ".$years."</h3>";
											   }
											   else{
											   	$bil = $conn->query("SELECT count(`pe_billnumber`) as cnt,SUM(`pe_gtotal`) as sm,EXTRACT(YEAR FROM pe_billdate) as yr,EXTRACT(MONTH FROM pe_billdate) as mnth FROM us_purentry WHERE pe_isactive='0' and finyear = '".$_SESSION["finyearid"]."' Group BY EXTRACT(MONTH FROM pe_billdate),EXTRACT(YEAR FROM pe_billdate) ORDER BY pe_billdate DESC");

                          $bil1 = $conn->query("SELECT count(`pe_billnumber`) as cnt,SUM(`pe_gtotal`) as sm FROM us_purentry WHERE pe_isactive='0' and pe_balance>0 and finyear = '".$_SESSION["finyearid"]."'  ORDER BY pe_billdate DESC");

                          $bil2 = $conn->query("SELECT count(`pe_billnumber`) as cnt,SUM(`pe_gtotal`) as sm FROM us_purentry WHERE pe_isactive='0' and pe_balance=0 and finyear = '".$_SESSION["finyearid"]."'  ORDER BY pe_billdate DESC");
												echo "<h3>ALL PURCHASE REPORTS</h3>";
											   }
								?>
                         ...
                         $row1 = $bil1->fetch_assoc();
                            ...
                            ?>
                                        ...
                                        <table class="table" border="0" style="height: 220px;">
                                          <tr style="width: 20px;">
                                            <td align="right"><strong><h3>Total Cash Purchase:</h3></strong></td>
                                            <td width="150" align="center"><h3>
                                              <?= $row2["cnt"] ?></h3>
                                            </td>
                                            </tr>
                                            <tr>
                                            <td align="right"><strong><h3>Total Credit Purchase:</h3></strong></td>
                                            <td width="150" align="center"><h3>
                                              <?= $row1["cnt"] ?></h3>
                                            </td>
                                            </tr>
                                            <tr>
                                            <td align="right"><strong><h3>Total Purchase:</h3></strong></td>
                                            <td width="150" align="center"><h3>
                                            	<?= $totsale ?></h3>
                                            </td>
                                            </tr>
                                             <tr>
                                            <td align="right"><strong><h3>Total Cash Purchase Amount:</h3></strong></td>
                                            <td width="150" align="center"><h3>
                                              <?= $row2["sm"] ?></h3>
                                            </td>
                                            </tr>
                                             <tr>
                                            <td align="right"><strong><h3>Total Credit Purchase Amount:</h3></strong></td>
                                            <td width="150" align="center"><h3>
                                              <?= $row1["sm"] ?></h3>
                                            </td>
                                            </tr>
