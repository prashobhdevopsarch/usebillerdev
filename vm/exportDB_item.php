...
if($fil == 'all')
{
	$sql = $conn->query("SELECT * FROM us_billentry WHERE user_id='".$_SESSION["admin"]."' AND be_isactive='0' ORDER BY be_billid DESC");
	$title= "All";
}
else{
	$filarr = explode('$', $fil);
	...
	$sql = $conn->query("SELECT * FROM us_billentry WHERE DATE(be_billdate)>='$fromdate' AND DATE(be_billdate) <= '$todate' AND be_isactive='0' AND user_id='".$_SESSION["admin"]."' ORDER BY be_billid DESC");
