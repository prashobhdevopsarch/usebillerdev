...
if($fil == 'all')
{
	$sql = $conn->query("SELECT * FROM us_billentry WHERE user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' AND be_isactive='0' AND be_mod='1' ORDER BY be_billid DESC");
	$title= "All";
}
else{
	$filarr = explode('$', $fil);
	...
	$sql = $conn->query("SELECT * FROM us_billentry WHERE DATE(be_billdate)>='$fromdate' AND DATE(be_billdate) <= '$todate' AND be_isactive='0' AND be_mod='1' AND user_id='".$_SESSION["admin"]."' and finyear = '".$_SESSION["finyearid"]."' ORDER BY be_billid DESC");
...
$output .=$title;
$output .="\n";
$output .='No.,Bill No.,Party,GSTIN/UIN,Bill Date,State Code,Items,HSN,KFC,Quantity,Taxable Amount,Tax %,Tax Amount,Total,Total Discount,Grand Total,';
$output .="\n";
...
$output.=$row3['pr_productname'].",".$row3['pr_hsn'].",".$row3['bi_cess'].",".$row3['bi_quantity'].",".$row3['bi_taxamount'].",".$row3['bi_vatper'].",".$row3['bi_vatamount'].",".$row3['bi_total'].",".$row['be_discount'].",".$row['be_gtotal'];
$k++;
$output .="\n";
$i=1;
$itmss = $conn->query("SELECT * FROM us_billitems LEFT JOIN  us_products  ON bi_productid= pr_productid WHERE bi_billid='$billid' LIMIT 1000 OFFSET 1");
   ...
   {
   $output .=", , , , , ,".$row3['pr_productname'].",".$row3['pr_hsn'].",".$row3['bi_cess'].",".$row3['bi_quantity']." ,".$row3['bi_taxamount']." ,".$row3['bi_vatper'].",".$row3['bi_vatamount'].",".$row3['bi_total'].", , , ";
   $output.="\n";
   $i++;}
   }
