                        ...
                        <div class="col-lg-12 col-md-12">
							...
							?>
                                <div class="panel-heading">
                                	<a href="exporthistoryp.php?fil=<?= $filt ?>" target="_blank"><button type="button" class="btn btn-primary btn-addon m-b-sm" style="float:right;"><span class="glyphicon glyphicon-print"></span> Print</button></a>
								...
								if(isset($_POST['filter']))
											   {
												   $fromdate = $_POST['fromdate'];
												   $todate = $_POST['todate'];
												   $bil = $conn->query("SELECT * FROM us_purentry a LEFT JOIN us_supplier b ON b.rs_supplierid=a.pe_supplierid WHERE DATE(a.pe_billdate)>='$fromdate' AND DATE(a.pe_billdate) <= '$todate' AND a.pe_isactive='0'AND a.user_id='".$_SESSION["admin"]."' ORDER BY a.pe_billid DESC");
												   echo "<h3>From Date: ".date('d-M-Y', strtotime($fromdate))." &nbsp; &nbsp; To Date: ".date('d-M-Y', strtotime($todate))."</h3>";
											   }
											   else{
											   	$bil = $conn->query("SELECT * FROM us_purentry a LEFT JOIN us_supplier b ON b.rs_supplierid=a.pe_supplierid WHERE a.pe_isactive='0' AND a.user_id='".$_SESSION["admin"]."' ORDER BY a.pe_billid DESC");
												echo "<h3>ALL PURCHASE DETAILS</h3>";
											   }
								?>
                                
                                    <div class="table-responsive project-stats">  
                                    <form class="form-horizontal" method="post" action="exportpurchase.php?fil=<?= $filt ?>">
                                       ...
                                       <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                                           ...
                                           <tbody>
                                               ...
                                               <tr>
                                                  ...
                                                  <a onClick="return confirm('Are you sure you want to delete?')" href="purchasehistory.php?billid=<?=$row['pe_billid']?>&delete&supid=<?=$rowsup1['pe_supplierid']?>&oldbal=<?=$row['rs_balance']?>"><span class="glyphicon glyphicon-trash"> <br></span> delete</a>
                                                   </td>
                                               </tr>
