...

{
	...
	$sql = $conn->query("SELECT * FROM us_billentry WHERE be_customerid='$customerid' AND  user_id='".$_SESSION["admin"]."' AND be_isactive='0' ORDER BY be_billid DESC");
	...
	$sql = $conn->query("SELECT * FROM us_billentry WHERE be_customerid='$customerid' AND  DATE(be_billdate)>='$fromdate' AND DATE(be_billdate) <= '$todate' AND be_isactive='0' AND user_id='".$_SESSION["admin"]."' ORDER BY be_billid DESC");
...
$output .=',Customer Bill History,';
$output .=$rowcus['cs_customername'];;
$output .="\n";
...
$filename = "CustomerReport".date('d-m-Y H:i').".csv";
