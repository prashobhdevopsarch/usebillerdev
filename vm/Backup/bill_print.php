...
table {
   ...
   /* background-image: url("../assets/srikrishna1.png");
...
.fnt td,.fnt th{border:1px solid grey; font-size:15px;}
...
<?php if(isset($_GET["back"])){?><a href="<?= $_GET["back"]?>"><button style="float:left;" class="printButtonClass">back</button></a><?php }else{?><a href="billinghistory.php"><button style="float:left;" class="printButtonClass">back</button></a><?php }?>
<?php 
session_start();
...
/*$change="update  us_billentry set be_typebill='original for customer' where be_billid='$billid'";
$res=$conn->query($change)or die(mysqli_error($conn));
$typeb='original for customer';*/
...
<!--<table  style="width:100%;"  style="border-bottom:1px solid">-->
	<!--<tr height="100px">-->
  	
   <!-- <th style="width:150px;"></th><th><span>INVOICE</span></th><br>-->
   <!--th rowspan="2" align="center" width=100% colspan="2"-->
  ...
  <!--</tr>
  <tr></tr>
  <tr><td colspan="2" align="center" style="border-left:1px solid #000;"><?=$row3["sp_shopaddress"]?><br> <?=$row3["sp_email"]?>&nbsp;&nbsp;
  <?=$row3["sp_mobile"]?></td>
 </tr>
  </table>-->
  ...
  <?php
  $sql=$conn->query("SELECT * FROM  us_shopprofile");
...
<table  width="100%" border="1">
  ...
  <tr border="1" >
    <td rowspan="3" style="padding:5px;width:75%; border-left:1px solid #000;text-align: center;">SALES INVOICE</td>
  ...
  <tr border="1" >
   ...
   <table style="text-align: left; border-color:#000;line-height:130%" width="100%" border="1" id="img">
   <tr border="1" >
    <td  style="padding:5px;width:50%; border-left:1px solid #000;">
                  <!--   Invoice Serial Number:  <b><?=$row4["be_billnumber"]?><br></b>
                    Invoice Date: <b><?=$row4["be_billdate"]?><br></b> -->
        ...
        Name:
        <b><?php
		echo $row4["be_customername"];?></b></div>
		<br>Address:
		<b><?php
		echo $row4["be_address"]; 
		?><br></b>Phone:
		<b><?php
        ...
        <br>State: <b><?php if($row4["be_statecode"]=="AN"){echo "Andaman and Nicobar Islands ";}
                         ...
                         else {echo "Tamil Nadu";}?></b>
        <br>
		<!--State Code:<b><?php /* if($row4["be_statecode"]==''){echo"32";$stcode_c='KL';}else{$stcode_c=$row4["be_statecode"];switch($row4["be_statecode"])
		{
		...
		<!--ID Number<b> <?php echo $row4["be_identification"];?><br>    -->       
        GST Number:  <b><?php echo $row4["be_customer_tin_num"];
		/*if($row4["be_shipping_address"]!='')
		{?>
		<br>  Shipping Address :  <b><?php echo $row4["be_shipping_address"];
		}
        } 
		else {?></b>
        Name:
		...
		?></b>
        <br>GST Number:  <b>--><?php echo $row5["cs_tin_number"];
		?></b>
					<!--<?php if($row4["be_shipping_address"]!='')
		{?>
		<br>  Shipping Address :  <b><?php echo $row4["be_shipping_address"];
		}
		}			
...
<!--table border="1" width="100%" class="table">
    ...
    <tr style="padding-left:20px;">
    	...
    	<td style="padding:5px; border-left:1px solid #000;line-height:150%;"> </td>
        
        <td style="padding:5px; border-left:1px solid #000;line-height:150%"> <?php if ($row4["be_customerid"] == "0") {
    	?>
        Name:
        <b><?php
...
{$clmspan='5';}
?>
<div style="border: 1px black solid;margin-bottom:0px;" class="nonborder">
<table width="100%" class="fnt" style="border-top: px white">
<tr>
<th  width="5%" bgcolor="#999999"> Sl.No</th>
<th  bgcolor="#999999" align="center" width="40%";>DESCRIPTION OF GOODS</th>
<th  width="10%" bgcolor="#999999">HSN CODE</th>
<th  width="9%" bgcolor="#999999">Qty</th>
<th  width="10%" bgcolor="#999999">GST %</th>
...
<th  width="10%" bgcolor="#999999">Total</th>
...
$tigst=0;
$taxtott=0;
$bill2=$conn->query("SELECT * FROM  us_billitems a LEFT JOIN  us_products b ON b.pr_productid=a.bi_productid WHERE a.bi_billid='$billid' LIMIT 14");
$tot=0;
while($row6=$bill2->fetch_assoc()){?>
<tr style="text-align:center; ">
    <td style="font-size:13px;"><?php echo $k;?></td>
    
    <td align="left" style="font-size:15px"><?php echo $row6['pr_productname'];?></td>
    <td border-bottom="0" style="font-size:13px;"><?php echo $row6['pr_hsn'];?></td>
    <td style="font-size:13px;"><?php echo $row6['bi_quantity'];?></td>
	 <td border-bottom="0"><?php echo $row6['bi_vatper'];?></td>
    <td style="font-size:13px;"><?php echo $row6['pr_unit'];?></td>
    <td style="font-size:13px;"><?php echo $row6['bi_price'];?></td>
    <td style="font-size:13px;"><?php echo $tottt=$row6['bi_price']*$row6['bi_quantity'];?></td>
    <!--<?php if($stcode_c==$stcode_s)
    {?>
...
<?php
    ...
    $su=$su+$row6['bi_price'];
    $p= $p+$row6['bi_taxamount']; 
    $tt=$tt+$tottt;
    $taxtott = $taxtott+$tottt;
    $cg=$cg+round($row6['bi_cgst_amt'],2);
    $sg=$sg+round($row6['bi_sgst_amt'],2);
    $ig=$ig+round($row6['bi_igst_amt'],2);
	$gt;
	$gt=round($row4['be_gtotal'] + $ig + $sg);
?>
<?php $k++; }
while($k<=12)
{?>
...
<tr style="border: 2px #000 solid">
 ...
 <td style="border-top: 2px #000 solid;border-bottom: 2px #000 solid; text-align:right;" colspan="2" align="right"><h3><b> </b></h3></td>
 <td style="border-top: 2px #000 solid;border-bottom: 2px #000 solid;"></td>
  <td style="border-top: 2px #000 solid;border-bottom: 2px #000 solid;text-align:center;"><?php echo $qt;?></td>
 <td style="border-top: 2px #000 solid; text-align:center;"></td>
 <td style="border-top: 2px #000 solid; text-align:center;"></td>
  <td style="border-top: 2px #000 solid; text-align:center;">SUB TOTAL</td> 
...
?>
 ...
 <td align="center">
 <!--<b><?php //echo $gt=round($row4['be_gtotal'] + $ig + $sg); ?></b> -->
 <b><?php echo $gt=round($row4['be_gtotal'] + $row6['bi_igst_amt'] + $row6['bi_sgst_amt'] + $row6['bi_cgst_amt'] ); ?></b>
 </td>
 </tr>
 <tr style="border: 2px #808080 solid;">
 <td style="display:none" colspan="7" align="center">
 <b>Frieght</b>
 </td>
 <td style="display:none" align="center">
 <b><?php echo $row4["be_coolie"]; ?></b>
 </td>
 </tr>
<tr style="border: 2px #808080 solid;">
 <td style="display:none" colspan="7" align="center">
 ...
 <td style="display:none" align="center">
 <b><?php echo $row4["be_discount"]; ?></b>
 </td>
 </tr>
 <tr style="border: 2px #808080 solid; text-align:left;" >
 
 <td width="100%" colspan="8" >
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b> <?php $gtotl=($tt); $amd=convert_number_to_words($row4['be_gtotal']);echo $amd;?> /- ONLY</b></td>
</tr>



 <tr style="border-top: 1px solid #808080 ;" >
  <td colspan="3"  rowspan="2" style="line-height:16px; font-size:12px; ">
 <u><b>Declaration:</b></u><br>
 We declare that this invoice shows the actual price of the goods described and that all particulars are true and correct


 
<b><u><br>Terms and Conditions:</br></u></b>
Goods once sold will not be taken back in any circumstances.<br>
We are not responsible for any damage or shortage of goods in transit.<br>
For any service and compliants kindly contact us with bill copy. Bill copy is mandatory.

               
				
 </td>
 
 <td colspan="5" style="font-size: 12px;"  style="line-height:16px;">
    <u><b>Bank Details</b></u><br>
	ABHISHEK ENTERPRISES<br>
      Bank Name:<?=$row3["sp_bank"]?><br>Branch:<?=$row3["sp_branch"]?>&nbsp;
       <br>Bank Account Number: <?=$row3["sp_accno"]?><br>IFSC Code: <?=$row3["sp_ifsc"]?>
 <hr>
...
<span style="float:right">For&nbsp;<?=$row3["sp_shopname"]?><br></span><br><br><br>Authorized Signature
 </td>
 </tr>
...
<b> Certified that the Particulars given above are true and correct</b>
 ...
 <center> This is a Computer Generated invoice<br>
...
{
	$r++;	
$slctitm1=$conn->query("SELECT * FROM  us_billitems a LEFT JOIN  us_products b ON b.pr_productid=a.bi_productid WHERE a.bi_billid='$billid' LIMIT ".$j.",14");
if($slctitm1->num_rows>0)
{
?>


<table style="text-align: left; border-color:#000;line-height:150%" width="100%" border="1">
   <tr border="1" >
    <td  style="padding:5px;width:50%; border-left:1px solid #000;">
      ...
      <!--  State Code:<b><?php //echo $row5["cs_statecode"];
		...
		<!--ID Number<b> <?php echo $row4["be_identification"];?><br> --> 
        <br>GST Number:  <b><?php echo $row5["cs_tin_number"];}?></b>
...
<?php
    ...
    $ig=$ig+round($row6['bi_igst_amt'],2);
	$gt;
	$gt=round($row4['be_gtotal'] + $row6['bi_igst_amt'] + $row6['bi_sgst_amt'] + $row6['bi_cgst_amt'] );
?>
<?php $k++; }
while($k<=12)
{?>
