...
<?php if(isset($_GET["back"])){?><a href="<?= $_GET["back"]?>"><button style="float:left;" class="printButtonClass">back</button></a><?php }else{?><a href="estimation.php"><button style="float:left;" class="printButtonClass">back</button></a><?php }?>
<?php 
session_start();
...
{
	$billid=$_GET["billid"];
	$slct=$conn->query("SELECT * FROM us_estimation WHERE bi_billid='$billid'");
	$row=$slct->fetch_assoc();
	
	$cnt=$conn->query("SELECT COUNT(bi_billitemid) AS cnt FROM us_estimationitems WHERE bi_billid='$billid'");
...
$bill = $conn->query("SELECT * FROM us_estimation WHERE bi_billid='$billid'");
$row4=$bill->fetch_assoc();
$cust= $conn->query("SELECT * FROM us_customer WHERE cs_customerid='".$row4["bi_customerid"]."'");
...
$totalbi1=0;
$gk2=0;
$bill2=$conn->query("SELECT * FROM us_estimationitems a LEFT JOIN us_products b ON b.pr_productid=a.bi_productid WHERE a.bi_billid='$billid' LIMIT 15");
$tot=0;
...
while($k<=15)
{?>
<tr><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td><td style="border-right:1px solid #808080;">&nbsp;</td></tr>
<?php $k++;}?>
...
<?php $slctsgst=$conn->query("select sum(bi_cgst_amt) as cgs,sum(bi_sgst_amt) as sgs,sum(bi_igst_amt) as igs  from us_estimationitems where bi_billid='$billid'"); $rowi=$slctsgst->fetch_assoc();
$totax=$rowi["cgs"]+$rowi["sgs"]+$rowi["igs"];?>

<span class="s8">Total Invoice Amount in words : <b> <?php $amd=convert_number_to_words(round($totalbi1));echo $amd;?> </b> Only</span>
</td>
...
<tr>
<td>
Transport Vehicle No. : <?=$row4["bi_vehicle_number"]?><br>
    <br>
    <b>Shipping Address : <?=$row4["bi_shipping"]?></br>
    <b>100% Advance : <?=$row4["bi_hunad"]?></br>
</td>
	<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8">Add:CGST</span>
</td>
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8"><?php $slctsgst5t=$conn->query("select sum(bi_cgst_amt) as 5tax from us_estimationitems where bi_billid='$billid'"); $rowitm5t=$slctsgst5t->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8"><?php $slctsgst6t=$conn->query("select sum(bi_sgst_amt) as 6tax from us_estimationitems where bi_billid='$billid'"); $rowitm6t=$slctsgst6t->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8"><?php $slcess=$conn->query("select sum(bi_cess) as b6cess from us_estimationitems where bi_billid='$billid'"); $rowitmcess=$slcess->fetch_assoc();
...
<td style="border-bottom:1px solid #000;" colspan="3">
<span class="s8">
<b><?= round($totalbi1+$row4["bi_coolie"]);?></b>
</span>
</td>
</tr>
...
{
	$r++;	
$slctitm1=$conn->query("SELECT * FROM us_estimationitems a LEFT JOIN us_products b ON b.pr_productid=a.bi_productid WHERE a.bi_billid='$billid' LIMIT ".$j.",15");
...
<table style="text-align: left; border-color:#000;" width="100%" border="1">
   <tr border="1" >
    <td  style="padding:5px;width:50%; border-left:1px solid #000;">
                    ...
                    Invoice Serial Number:  <b><?=$row4["bi_billnumber"]?>(<?php $d=date('d-m',strtotime($row4["bi_billdate"]));echo $d;?>)</b><br>
                    Invoice Date: <?php $dt=date('d-m-y',strtotime($row4["bi_billdate"]));echo $dt?><br>
    ...
    <td  style="padding:5px;width:50%; border-left:1px solid #000">Transportation Mode:<br>
                    Vehicle Number: <?=$row4["bi_vehicle_number"]?><br>
                    Date And Time Of Supply:<?php $dt=date('d-m-y',strtotime($row4["bi_billdate"]));echo $dt?><br>
                    Place Of Supply: <b><?php if ($row4["bi_customerid"] == "0") { 
                         if($row4["bi_statecode"]=="AN"){echo "Andaman and Nicobar Islands ";}
                         elseif($row4["bi_statecode"]=="AP"){echo "Andhra Pradesh";}
                         elseif($row4["bi_statecode"]=="AD"){echo "Andhra Pradesh";}                                                                                         elseif($row4["bi_statecode"]=="AR"){echo "Arunachal Pradesh";}
                         elseif($row4["bi_statecode"]=="AS"){echo "Assam";}
                         elseif($row4["bi_statecode"]=="BH"){echo "Bihar";}
                         elseif($row4["bi_statecode"]=="CH"){echo "Chandigarh";}
                         elseif($row4["bi_statecode"]=="CT"){echo "Chattisgarh";}
                         elseif($row4["bi_statecode"]=="DN"){echo "Dadra and Nagar Haveli";}
                         elseif($row4["bi_statecode"]=="DD"){echo "Daman and Diu";}
                         elseif($row4["bi_statecode"]=="DL"){echo "Delhi";}
                         elseif($row4["bi_statecode"]=="GA"){echo "GoA";}
                         elseif($row4["bi_statecode"]=="GJ"){echo "Gujarat";}
                         elseif($row4["bi_statecode"]=="HR"){echo "Hariyana";}                           
                         elseif($row4["bi_statecode"]=="HP"){echo "Himachal Pradesh";}
                         elseif($row4["bi_statecode"]=="JK"){echo "Jammu And Kashmir";}
                         elseif($row4["bi_statecode"]=="JH"){echo "Jharkhand";}
                         elseif($row4["bi_statecode"]=="KA"){echo "Karnataka";}
                         elseif($row4["bi_statecode"]=="KL"){echo "Kerala";}
                         elseif($row4["bi_statecode"]=="LD"){echo "LakshadWeep Island";}
                         elseif($row4["bi_statecode"]=="MP"){echo "Madhya Pradhesh";}
                         elseif($row4["bi_statecode"]=="MH"){echo "Maharastra";}
                         elseif($row4["bi_statecode"]=="MN"){echo "Manipur";}
                         elseif($row4["bi_statecode"]=="ME"){echo "Meghalaya";}
                         elseif($row4["bi_statecode"]=="MI"){echo "Mizoram";}                           
                         elseif($row4["bi_statecode"]=="NL"){echo "Nagaland";}
                         elseif($row4["bi_statecode"]=="OR"){echo "Odisha";}
                         elseif($row4["bi_statecode"]=="PY"){echo "Pondicherry";}
                         elseif($row4["bi_statecode"]=="PB"){echo "Punjab";}
                         elseif($row4["bi_statecode"]=="RJ"){echo "Rajasthan";}                           
                         elseif($row4["bi_statecode"]=="SK"){echo "Sikkim";}
                         elseif($row4["bi_statecode"]=="TN"){echo "Tamil Nadu";}
                         elseif($row4["bi_statecode"]=="TS"){echo "Telangana";}
                         elseif($row4["bi_statecode"]=="TR"){echo "Tripura";}
                         elseif($row4["bi_statecode"]=="UP"){echo "Uttar Pradesh";}
                         elseif($row4["bi_statecode"]=="UI"){echo "Uttarakhand";}
                         elseif($row4["bi_statecode"]=="WB"){echo "West Bengal";}
                         else {echo "Kerala";}}
                    else{
...
<table border="1" width="100%" class="table">
    ...
    <tr>
        ...
        <td style="padding:5px; border-left:1px solid #000;"> <?php if ($row4["bi_customerid"] == "0") {
        ?>
        Name:
        <b><?php
        echo $row4["bi_customername"];?></b>
        <br>Ph:
        <b><?php
        echo $row4["bi_customermobile"]; 
        ?></b>
        <br>State: <b><?php if($row4["bi_statecode"]=="AN"){echo "Andaman and Nicobar Islands ";}
                         elseif($row4["bi_statecode"]=="AP"){echo "Andhra Pradesh";}
                         elseif($row4["bi_statecode"]=="AD"){echo "Andhra Pradesh";}                                                                                         elseif($row4["bi_statecode"]=="AR"){echo "Arunachal Pradesh";}
                         elseif($row4["bi_statecode"]=="AS"){echo "Assam";}
                         elseif($row4["bi_statecode"]=="BH"){echo "Bihar";}
        ...
        <br>State Code:<b><?php if($row4["bi_statecode"]==''){echo"32";$stcode_c='KL';}else{$stcode_c=$row4["bi_statecode"];switch($row4["bi_statecode"])
        {
        ...
        GST Number:  <b><?php echo $row4["bi_customer_tin_num"];
        } 
        else {?></b>
        Name:
        <b><?php
        ...
        <br>State Code:<b><?php if($row4["bi_statecode"]==''){$stcode_c='KL';echo"32";}else{$stcode_c=$row4["bi_statecode"];switch($row4["bi_statecode"])
        {
            case 'AN':echo "35";break;
            case 'AP':echo "28";break;
            case 'AD':echo "37";break;
        ...
        else {$cust= $conn->query("SELECT * FROM us_customer WHERE cs_customerid='".$row4["bi_customerid"]."'");
        ...
        ?></b>
         ...
         <br>
        State Code: <b><?php //echo $row5["cs_statecode"];
        $row4["bi_statecode"]=$row5["cs_statecode"];
        switch($row5["cs_statecode"])
        {
            case 'AN':echo "35";break;
            case 'AP':echo "28";break;
...
$gk2=0;
$tot=0;
$bill2=$conn->query("SELECT * FROM us_estimationitems a LEFT JOIN us_products b ON b.pr_productid=a.bi_productid WHERE a.bi_billid='$billid' LIMIT ".$j.",20");
...
<td style="border-bottom:1px solid #000;" colspan="2">
<span class="s8"><?php $slctsgst7t=$conn->query("select sum(bi_igst_amt) as 7tax from us_estimationitems where bi_billid='$billid'"); $rowitm7t=$slctsgst7t->fetch_assoc();
...
<tr>
<td style="border-bottom:1px solid #000;" colspan="2">Other Charges</td><td style="border-bottom:1px solid #000;" colspan="2"><?=$row4["bi_coolie"]?></td>
</tr>

<tr>
<td style="border-bottom:1px solid #000;" colspan="2"><b>Invoice Total(Rs) </b></td><td style="border-bottom:1px solid #000;" colspan="2"><b><?= round($totalbi2+$row4["bi_coolie"]);?></b></td>
</tr>
<?php
    $billtot = $conn->query("SELECT sum(bi_total) as grandtotal FROM us_estimationitems WHERE bi_billid='$billid'");
