...
if(isset($_SESSION['admin']))
{
	...
	{
		$cusid=$_GET["cusid"];
		$delete=$conn->query("DELETE FROM us_customer WHERE cs_customerid='$cusid'");
		if($delete)
		{
							...
							?>
                                <div class="panel-heading">
								 <h4 class="panel-title">Customer List</h4>
                                	<a href="customer.php"><button type="button" class="btn btn-primary btn-addon m-b-sm" style="float:right"><i class="fa fa-plus"></i> Add New Customer</button></a>
								...
								if(isset($_POST['filter']))
											   {
												   $fromdate = $_POST['fromdate'];
												   $todate = $_POST['todate'];
												   $bil = $conn->query("SELECT * FROM us_customer WHERE DATE(cs_billdate)>='$fromdate' AND DATE(cs_billdate) <= '$todate' AND cs_isactive='0' AND user_id='".$_SESSION["admin"]."' ORDER BY cs_customerid DESC");
												   echo "<h3>From Date: ".date('d-M-Y', strtotime($fromdate))." &nbsp; &nbsp; To Date: ".date('d-M-Y', strtotime($todate))."</h3>";
											   }
											   else{
											   	$bil = $conn->query("SELECT * FROM us_customer WHERE cs_isactive='0' AND user_id='".$_SESSION["admin"]."' ORDER BY cs_customerid DESC");
												echo "<h3>ALL CUSTOMER DETAILS</h3>";
											   }
								?>
                                           ...
                                           <tbody>
                                               ...
                                               <tr>
                                                  ...
                                                  <td><?=$row['cs_address']?></td>
                                                   ...
                                                   /*$balance=$conn->query("SELECT SUM(be_balance) AS totbal FROM us_billentry WHERE user_id='".$_SESSION["admin"]."' AND be_isactive='0' AND be_customerid='".$row['cs_customerid']."'");
												...
												$totalamnt=$totalamnt+$row['cs_balance'];
												  ...
												  ?> 
                                                   ...
                                                   <td><a href="view.php?csid=<?=$row['cs_customerid']?>"><i class="fa fa-eye" aria-hidden="true"></i>&nbsp;view</a> <br><a href="editcustomer.php?cusid=<?=$row['cs_customerid']?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>&nbsp;edit</a>
												   <br><a onClick="return confirm('Are you sure')" href="cushistory.php?cusid=<?=$row['cs_customerid']?>&delet"><i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;delete</a>
