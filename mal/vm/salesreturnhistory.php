...
if(isset($_SESSION['admin']))
{
	...
	{
		$bill_id=$_GET["billid"];
		$delete=$conn->query("UPDATE vm_salreturnentry SET sre_isactive='1' WHERE sre_billid='$bill_id'");
        ...
        <main class="page-content content-wrap">
            ...
            <div class="page-inner">
                <div class="page-title">
                    <h3><strong>സെയിൽസ് റിട്ടേൺ ഹിസ്റ്ററി</strong></h3>
                    <div class="page-breadcrumb">
                        <ol class="breadcrumb">
                            <li>ബില്ലിംഗ്</li>
                            <li class="active">സെയിൽസ് റിട്ടേൺ ഹിസ്റ്ററി</li>
                        </ol>
                    </div>
                </div>
                <?php
                        ...
                        <div class="col-lg-12 col-md-12">
							...
							?>
                                <div class="panel-heading">
                                	<a href="exporthistorysr.php?fil=<?= $filt ?>" target="_blank"><button type="button" class="btn btn-primary btn-addon m-b-sm" style="float:right;"><span class="glyphicon glyphicon-print"></span> പ്രിൻറ്</button></a>
                                    
                                    <h4 class="panel-title">സെയിൽസ് റിട്ടേൺ ഹിസ്റ്ററി</h4>
                                </div>
								...
								if(isset($_POST['filter']))
											   {
												   $fromdate = $_POST['fromdate'];
												   $todate = $_POST['todate'];
												   $bil = $conn->query("SELECT * FROM vm_salreturnentry a LEFT JOIN vm_customer b ON b.cs_customerid=a.sre_supplierid WHERE DATE(a.sre_billdate)>='$fromdate' AND DATE(a.pe_billdate) <= '$todate' AND a.sre_isactive='0'AND a.user_id='".$_SESSION["admin"]."' ORDER BY a.sre_billid DESC");
												   echo "<h3>From Date: ".date('d-M-Y', strtotime($fromdate))." &nbsp; &nbsp; To Date: ".date('d-M-Y', strtotime($todate))."</h3>";
											   }
											   else{
											   	$bil = $conn->query("SELECT * FROM vm_salreturnentry a LEFT JOIN vm_customer b ON b.cs_customerid=a.sre_supplierid WHERE a.sre_isactive='0' AND a.user_id='".$_SESSION["admin"]."' ORDER BY a.sre_billid DESC");
												echo "<h3>ഓൾ സെയിൽസ് റിട്ടേൺ ഡീറ്റെയിൽസ്</h3>";
											   }
								?>
                                       ...
                                       <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                                           ...
                                           <tbody>
                                               ...
                                               <tr>
                                                  ...
                                                  <td><?= $row['sre_rebill'] ?></td>
                                                   <td>
													...
													$itms = $conn->query("SELECT * FROM vm_salreturnitem a LEFT JOIN vm_products b ON b.pr_productid=a.sri_productid WHERE a.sri_billid='$billid'");
													$n=1;
                                              ...
                                              <!--     <td><?php
													   ...
													   ?></td>  -->
                                                   <td><a href="salesreturn_print.php?billid=<?=$row['sre_billid']?>&back=<?=$page?>"><span class="glyphicon glyphicon-print"></span> പ്രിൻറ്</a><br>
                                                 <!--  <a href=".php?billid=<?=$row['sre_billid']?>"><span class="glyphicon glyphicon-edit"></span> എഡിറ്റ്</a>-->
                                                  <a onClick="return confirm('Are you sure you want to delete?')" href="salesreturnhistory.php?billid=<?=$row['sre_billid']?>&delete"><span class="glyphicon glyphicon-trash"></span> ഡിലീറ്റ്</a>
                                                   </td>
                                               </tr>
